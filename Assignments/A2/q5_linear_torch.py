...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        h = torch.flatten(state, start_dim=1)
        out = self.q_network(h) if network=='q_network' else self.target_network(h)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        Q_samp = torch.where(done_mask, rewards, rewards + gamma * torch.max(target_q_values, axis=1).values)
        Q = torch.sum(torch.nn.functional.one_hot(actions.to(torch.int64), num_actions)* q_values, axis=1)
        return torch.nn.functional.mse_loss(Q_samp, Q)
        ##############################################################
        ######################## END YOUR CODE #######################
