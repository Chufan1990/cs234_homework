	...
	while True:
		...
		for s, a in enumerate(policy):
			...
			for prob, nextstate, reward, _ in P[s][a]:
				r = reward
				dv += prob * value_function[nextstate]
			value_function[s] = r + gamma * dv
			diff = abs(value_function[s] - old_v)
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for s in range(nS):
		...
		for a in range(nA):
			...
			for prob, nextstate, reward, _ in P[s][a]:
				r = reward
				dq += prob * value_from_policy[nextstate]
			q[a] = r + gamma * dq
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	while True:
		...
		if not ((policy - old_policy) > 0).any():
			break
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	# YOUR IMPLEMENTATION HERE #
	while True:
		old_v = np.copy(value_function)
		for s in range(nS):
			q = np.zeros(nS, dtype='float')
			for a in range(nA):
				r = 0
				dq = 0
				for prob, nextstate, reward, _ in P[s][a]:
					r = reward
					dq += prob * value_function[nextstate]
				q[a] = r + gamma * dq
			value_function[s] = np.max(q)
			policy[s] = np.argmax(q)

		if np.max(np.abs(value_function - old_v)) < tol:
			break
